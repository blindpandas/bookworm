...
class EpubDocument(SinglePageDocument):
    ...
    @cached_property
    def language(self):
        content = (file.content for file in self.epub.get_items_of_type(ebooklib.ITEM_DOCUMENT))
    ...
    def get_cover_image(self):
        if not (cover := more_itertools.first(self.epub.get_items_of_type(ebooklib.ITEM_COVER), None)):
        ...
        for sect in self.add_toc_entry(self.epub.toc, root):
            ...
            try:
                ...
                if text_range is None:
                    log.warning(f"Could not determine the starting position for href: {href} and section: {sect!r}")
    ...
    def start_positions_for_sections(self):
        sect_starting_poses =  [(0, self.toc_tree),] + [
            (sect.text_range.start, sect)
            for sect in self.toc_tree.iter_children()
        ]
        data = list(more_itertools.zip_offset(
            sect_starting_poses,
            sect_starting_poses,
            offsets=(0, 1),
            longest=True
