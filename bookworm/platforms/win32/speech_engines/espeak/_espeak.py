        ...
        for e in event:
            if e.type == espeakEVENT_MARK:
                try:
                    ...
                    onIndexReached(decodeEspeakString(e.id.name))
                    continue
...
def _setVoiceAndVariant(voice=None, variant=None):
    ...
    if variant == "none":
        ...
        try:
            espeakDLL.espeak_SetVoiceByName(
                encodeEspeakString("%s+%s" % (voice, variant))
            )
        except:
            espeakDLL.espeak_SetVoiceByName(encodeEspeakString(voice))
...
def initialize(indexCallback=None):
    ...
    espeakDLL = cdll.LoadLibrary(os.path.join(get_espeak_dll_path()))
    ...
    eSpeakPath = get_espeak_directory()
...
@cache
def getVariantDict():
    dir = os.path.join(get_espeak_directory(), "espeak-ng-data", "voices", "!v")
    ...
    for fileName in os.listdir(dir):
        ...
        if os.path.isfile(absFilePath):
            ...
            try:
                ...
                log.error(
                    "Couldn't parse espeak variant file %s" % fileName, exc_info=True
                )
                continue
        if name is not None:
