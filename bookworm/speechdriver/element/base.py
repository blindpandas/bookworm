...
from .enums import EmphSpec, RateSpec, SpeechElementKind, VolumeSpec
...
class SpeechStyle:
    ...
    def start_style_decompose(self):
        if self.voice:
            self.__close_decompose.append(
                SpeechElement(SpeechElementKind.end_voice, None)
            )
            yield SpeechElement(SpeechElementKind.start_voice, self.voice)
        if self.emph:
            self.__close_decompose.append(
                SpeechElement(SpeechElementKind.end_emph, None)
            )
            yield SpeechElement(SpeechElementKind.start_emph, self.emph)
        prosody_vals = (self.pitch, self.rate, self.volume)
        if any(prosody_vals):
            self.__close_decompose.append(
                SpeechElement(SpeechElementKind.end_prosody, prosody_vals)
            )
